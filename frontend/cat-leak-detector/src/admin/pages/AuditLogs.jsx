...
import { Shield, Search, Download, Trash2, Clock, ChevronDown } from 'lucide-react';

const FONT = { fontFamily: "'Inter', 'Segoe UI', Arial, sans-serif" };

// TODO: Replace with backend API calls
// API ENDPOINTS: /api/audit-logs (GET), /api/audit-logs (DELETE)
      ...
      <div className="p-6" style={FONT}>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <button onClick={exportLogs} className="flex items-center gap-2 px-4 py-2 border border-gray-200 rounded-lg text-xs font-semibold text-gray-600 hover:bg-gray-50 transition-colors">
              <Download className="w-3.5 h-3.5" /> Export CSV
            </button>
            {currentAdmin?.role === 'Admin' && (
              <button onClick={handleClearLogs} className="flex items-center gap-2 px-4 py-2 border border-red-200 rounded-lg text-xs font-semibold text-red-600 hover:bg-red-50 transition-colors">
                ...
                {paginated.length === 0 ? (
                  ...
                  <tr key={i} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      <span className={`inline-flex px-2 py-0.5 rounded text-[9px] font-semibold uppercase ${actionColors[log.action] || 'bg-gray-100 text-gray-700'}`}>
                        {log.action}
                      </span>
                    </td>
                ...
                {Array.from({ length: totalPages }, (_, i) => (
                  <button key={i} onClick={() => setCurrentPage(i + 1)}
                    className={`px-3 py-1 text-xs font-semibold rounded transition-colors ${
                      currentPage === i + 1 ? 'bg-[#FFCD11] text-[#111111]' : 'text-gray-500 hover:bg-gray-100'
                    }`}>
                    {i + 1}
                  </button>
                ))}

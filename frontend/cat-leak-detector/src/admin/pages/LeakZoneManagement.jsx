...
export default function LeakZoneManagement() {
  ...
  const handleSave = () => {
    ...
    if (editingZone) {
      ...
      addAuditLog(currentUser, 'Update Leak Zone', `Updated zone: ${editingZone.id}`);
    } else {
      addLeakZone(payload);
      addAuditLog(currentUser, 'Add Leak Zone', `Added zone: ${payload.name}`);
    }
  ...
  const handleDelete = (zone) => {
    if (window.confirm(`Delete leak zone "${zone.name}"?`)) {
      ...
      setZones(getLeakZones());
      addAuditLog(currentUser, 'Delete Leak Zone', `Deleted zone: ${zone.id}`);
    }
  };
      ...
      <div className="p-4 lg:p-6" style={FONT}>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4">
          <div className="relative">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search zones..." className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-200 bg-gray-50 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cat-yellow/50" />
          </div>
        </div>
            ...
            <div key={i} className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow group">
              <div className="h-40 bg-gray-100 flex items-center justify-center relative overflow-hidden">
                ...
                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => openEditModal(zone)} className="p-1.5 rounded bg-white/90 text-gray-600 hover:text-blue-500 shadow cursor-pointer"><Edit className="w-3.5 h-3.5" /></button>
                  <button onClick={() => handleDelete(zone)} className="p-1.5 rounded bg-white/90 text-gray-600 hover:text-red-500 shadow cursor-pointer"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </div>
              <div className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-sm font-bold text-gray-900">{zone.name}</h3>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-[9px] font-semibold uppercase ${
                ...
                <div className="flex items-center justify-between">
                  <span className="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold bg-cat-yellow/10 text-cat-yellow border border-cat-yellow/30">{zone.engineModel}</span>
                  <span className="text-[9px] text-gray-400">{new Date(zone.updatedAt).toLocaleDateString()}</span>
                </div>
              </div>
            </div>
          ))}
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-semibold uppercase tracking-widest text-gray-500 mb-1">Zone Name *</label>
                ...
                <div>
                  <label className="block text-[10px] font-semibold uppercase tracking-widest text-gray-500 mb-1">Zone Type</label>
                  <select value={formData.zoneType} onChange={e => setFormData(p => ({ ...p, zoneType: e.target.value }))}
                    className="w-full px-3 py-2.5 rounded-lg border border-gray-200 bg-gray-50 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-cat-yellow/50">
                    <option value="Intake">Intake</option><option value="Exhaust">Exhaust</option><option value="Both">Both</option>
                  </select>
                </div>
              </div>
              <div>
                ...
                <textarea value={formData.description} onChange={e => setFormData(p => ({ ...p, description: e.target.value }))} rows={3}
                  className={`w-full px-3 py-2.5 rounded-lg border text-sm ${formErrors.description ? 'border-red-300' : 'border-gray-200'} bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-cat-yellow/50`} />
              </div>
              <div>
                <label className="block text-[10px] font-semibold uppercase tracking-widest text-gray-500 mb-1">Leak Zone Diagram</label>
                <div className="border-2 border-dashed border-gray-300 rounded-xl p-4 text-center">
                  {previewUrl ? (
                    <div className="space-y-2">
                      <img src={previewUrl} alt="Preview" className="max-h-40 mx-auto rounded-lg" />
                      <button onClick={() => { setPreviewUrl(null); setFormData(p => ({ ...p, imageUrl: '' })); }} className="text-xs text-red-500 hover:text-red-600 cursor-pointer">Remove</button>
                    </div>
                  ) : (
                    <div>
                      <Upload className="w-8 h-8 text-gray-300 mx-auto mb-2" />
                      <p className="text-xs text-gray-500 mb-2">Drag & drop or click to upload diagram</p>
